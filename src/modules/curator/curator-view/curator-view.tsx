...
const CuratorCabinet = () => {
	...
	const [
		isEditDescriptionModalSubmitLoading,
		setIsEditDescriptionModalSubmitLoading,
	] = useState<boolean>(false)
	const [editingDescription, setEditingDescription] = useState<Student | null>(
		null
	)
	...
	const [isCreateStudentModalVisible, setIsCreateStudentModalVisible] =
		useState<boolean>(false)
	const [
		isCreateStudentModalSubmitLoading,
		setIsCreateStudentModalSubmitLoading,
	] = useState<boolean>(false)
	...
	const loadStudents = async ({ page = 1 } = { page: 1 }) => {
		...
		try {
			const [fetchedStudents, fetchedPaginationMeta] =
				await unauthorizedHandler(studentsRepository.getStudents({ page }))
	...
	const handleCreateStudentModalSubmit = async (payload: {
		name: string
		surname: string
		state: number
		description: string
		...
		try {
			await unauthorizedHandler(studentsRepository.createStudent(payload))
		...
		<div className={styles.wrapper}>
			...
			<EditDescriptionModal
				initialData={{
					...
					performance: editingDescription?.performance ?? 1,
